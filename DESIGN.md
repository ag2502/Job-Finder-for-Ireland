...
- **The first screen (981px and up)** is the desk plus the dock, sized to the window (`100svh` less the menu bar), so the dock of companies hiring is always visible without scrolling on a laptop. Careers windows sit in two side columns placed by share of the desk's height and shrink to as little as 0.66 on short screens; the two top-centre windows and the scribble need 940px of height, the stickers 880px.
- **The how-it-works story plays once, downwards.** It is pinned for 250vh and all three steps are always readable (the active one brightens); the demo reacts within about 0.1 to 0.3s. Once it has been scrolled past, or skipped with Find my jobs, it folds into an ordinary section with every step ticked green, with the scroll position corrected so nothing on screen moves; scrolling back up passes through it in under a screen.
